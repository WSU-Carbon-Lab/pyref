...
class RSOXS_Scan(ALS_MeasurementWidget):
    ...
    def generate_escan(self, input_d):
        ...
        output = getattr(self, "escan_info")
...
class RSOXS_ScriptGen(ALS_ScriptGenWidget):
    ...
    def __init__(self, save_path="", escan_path=DEFAULT_ESCAN_PATH, **kwargs):
        ...
        self.select_col_title = widgets.HTML(value=f"<b>Select optional motors to run</b>")
    ...
    def save_script(self, ext='.txt', sep='\t', b=None):
        ...
        for i, (index, detector) in enumerate(df.items()):
            ScanList = [s for s in detector.keys() if "scan_" in s]
            number_of_samples = len(ScanList)
            ...
            for ii, scan in enumerate(ScanList):
                ...
                motors_to_run.extend([motor for motor in self.select_cols.value])
...
class RSOXS_MacroGen(RSOXS_ScriptGen):
    ...
    def save_script(self, ext='.txt', sep='\t', b=None):
        ...
        if self.post_run_nexafs.value: # Run NEXAFS after cycle?
            macro += [add_comment(f"Running NEXAFS")]
            NEXAFS_script = SAVEDIR + '/' + "NEXAFS_script.txt"
            dnexafs = df[f'det_{pad_digits(1)}']
            ScanList = [s for s in dnexafs.keys() if "scan_" in s] # Get all the scans for the first detector position
    ...
    def save_spiral(self, ext='.txt.', sep='\t', b=None):
        SAVEDIR = str(self.save_dir)
        SAVENAME = str(self.save_name.value)
        SAVEPATH = SAVEDIR + '/' + SAVENAME + '_spiral.txt'
    ...
    def _build_individual_scan_files(self, df):
        ...
        for i, (index, detector) in enumerate(df.items()):
            ...
            ScanList = [s for s in detector.keys() if "scan_" in s] # Get all the scans in the current detector position
    ...
    def _build_spiral_scan(self, df):     
        ...
        for i, scan in enumerate(ScanList):
            ...
            dm = {key: value for key, value in detector[scan].items() if key != "name"}
            ...
            for ii, point in enumerate(grid):
