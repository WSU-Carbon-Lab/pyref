...
def check_struct(structure):
    if "layers" not in structure:
        ...
        typer.secho(
            "[Vacuum] | [layerN] | ... | [layer1] | [C] | [SiO2] | [Si]\n".center(
                typer.get_terminal_size().columns
            ),
            fg=typer.colors.BRIGHT_MAGENTA,
            bold=True,
        )
        layer_str = typer.prompt(
            "Enter the name layers in the model", type=str, default=""
        )
        if layer_str == "":
...
def check_single_layer(structure: dict, layer: str):
    ...
    if "thickness" not in structure[layer]:
        typer.secho(
            "Thickness missing from the structure", fg=typer.colors.RED, bold=True
        )
        ...
        typer.secho(
            f"Thickness: {structure[layer]['thickness']}",
            fg=typer.colors.BLACK,
            bold=True,
        )

    if "roughness" not in structure[layer]:
        typer.secho(
            "Roughness missing from the structure", fg=typer.colors.BLACK, bold=True
        )
        roughness = typer.prompt(
            "Enter the roughness of the layer",
            type=float,
            default=structure[layer]["thickness"] / 2,
        )
        ...
        typer.secho(
            f"Roughness: {structure[layer]['roughness']}",
            fg=typer.colors.BLACK,
            bold=True,
        )

    if "density" not in structure[layer]:
        typer.secho(
            "Density missing from the structure", fg=typer.colors.RED, bold=True
        )
        ...
        typer.secho(
            f"Density: {structure[layer]['density']}", fg=typer.colors.BLACK, bold=True
        )

    typer.secho(
        "-" * typer.get_terminal_size().columns, fg=typer.colors.BLACK, bold=True
    )
...
def write_json(path, editor, generate=True):
    ...
    typer.secho("Locating structure file... \n", fg=typer.colors.GREEN, bold=True)
    if generate is True:
        example = Path("example_struct.json")
        shutil.copy2(example, path)

    typer.pause("Compleated! press enter to edit...")
    pf = platforms()
    ...
    if editor is None:
        editor = typer.prompt("Editor", default="code")
    ...
    if is_executable_on_path(editor):
        ...
        typer.secho(
            f"Please install {editor} and try again", fg=typer.colors.RED, bold=True
        )
...
def structure_gen_interface(structure):
    ...
    check_struct(structure)
    typer.secho(
        f"Slab Structure: {" | ".join(structure["layers"])}",
        fg=typer.colors.BRIGHT_MAGENTA,
        bold=True,
    )
...
def generate(
    ...
    * Density
    ...
    if touch is True:
        ...
        print(
            f":sparkles: [bold green]STRUCTURE SAVED TO {file_loc}[/bold green] :sparkles:"
        )
    ...
    * Density
    ...
    if load is not None:
        ...
        if touch is True:
            ...
            print(
                f":sparkles: [bold green]STRUCTURE SAVED TO {load}[/bold green] :sparkles:\n"
            )
            typer.secho("Validating ...", fg=typer.colors.GREEN, bold=True)
            structure_gen_interface(structure)

        else:
            typer.secho(
                f"Structure loaded from {load}", fg=typer.colors.GREEN, bold=True
            )
            print(structure)
            structure_gen_interface(structure)

        print(":sparkles: [bold green]STRUCTURE VALIDATED[/bold green] :sparkles:")
