...
default_variable_motors = ['Higher Order Suppressor', 'Horizontal Exit Slit Size', 'EXPOSURE']
...
class PXRImage:
    ...
    def update_args(self, **kwargs):
        ...
        self.energy_offset = kwargs.get("energy_offset", getattr(self, 'energy_offset', 0))
        self.exposure_offset = kwargs.get("exposure_offset",getattr(self, 'exposure_offset', 0))
        ...
        self.beam_current = self.meta[beamcurrent_motor] if self.meta[beamcurrent_motor] > 50 else 1 # Change to 1 if it did not record
        self.polarization = self.meta[pol_motor]
        
        # Methods used to filter image and locate beam
        self._filter = kwargs.get("filter_func", getattr(self, '_filter', ndimage.median_filter))
        self._filter_kwargs = kwargs.get("filter_kwargs", getattr(self, '_filter_kwargs', {"size":3}))
        self._locate_beam_method = kwargs.get("loc_method", getattr(self, '_locate_beam_method', "argmax"))
    ...
    def image(self, image):
        if not isinstance(image, np.ndarray):
            raise TypeError(f"Image must be a 2D Numpy array, but got {type(value).__name__} instead.")
        try:
            self._image = image[self.dx:-self.dx, self.dy:-self.dy]
        except AttributeError:
            self._image = image

    def process_image(self, **kwargs): # Process the image given **kwargs from the loader.
    ...
    def height(self, value):
        if not isinstance(value, int):
            warnings.warn(f"Height must be an int, but got {type(value).__name__}. Converting to int.", UserWarning)
            try:
    ...
    def width(self, value):
        if not isinstance(value, int):
            warnings.warn(f"Width must be an int, but got {type(value).__name__}. Converting to int.", UserWarning)
            try:
                value = int(np.round(value,0))
            except ValueError:
    ...
    def edge(self, value):
        if not isinstance(value, tuple):
            if not isinstance(value, int):
                warnings.warn(f"Edge trim must be an int, but got {type(value).__name__}. Converting to int.", UserWarning)
                try:
    ...
    def update_spot(self):
        ...
        y_spot, x_spot = locate_peak(temp_image, self._locate_beam_method, **self._locate_beam_dict)
        ...
        rectangle = plt.Rectangle((x_low_bound, y_low_bound), w, h, edgecolor='yellow', facecolor='None')
...
class PrsoxrLoader:
    ...
    def __init__(self, files, name=None, mask=None, autoload=True):
        ...
        self.variable_motors = copy.deepcopy(default_variable_motors) # Now including EXPOSURE
        ...
        if autoload:
            for i, file in enumerate(self.files):
                ...
                self.PXRImages.append(PXRImage(image=image_i, metadata=meta_i, **self.image_kwargs)) # Store and process data
                self._images.append(image_i)
                self._meta_list.append(meta_i) 
            # Create a compiled dataframe of metadata.#
            self._meta_dict = {key: [d[key] for d in self._meta_list] for key in self._meta_list[0].keys()}
    ...
    def images(self, energy=None, polarization=None):
        ...
        if polarization == None:
            polarization = float(np.round(df[pol_motor].value_counts().idxmax(),pol_resolution)) # Get the polarization that shows up the most (default)
        # Find the energies that matches
        df['difference'] = abs(np.round(df[energy_motor],energy_resolution)-energy) # Find the closest value to the given energy
        df['difference_pol'] = abs(np.round(df[pol_motor],pol_resolution)-polarization)
    ...
    def optimize_box(self, dim_list=np.arange(1,42,2), frame=0, tol=0.9, replace=False, d=1):
        """
        ...
        for x in dim_list:
            ...
            self.height = int(x)
            self.width = int(x)
            temp_image = PXRImage(self._images[frame], self._meta_list[frame], autoload=True, **self.image_kwargs)
        ...
        if d:
            ...
            ax.plot(dim_list, i0_calc, ms=3, marker='x', lw=1, color='black', label='Total Counts')
            ax.axvline(x=opt_size, color = 'r')
            #ax.plot(opt_size, max_count*tol, ms=5, marker='o', lw=0, color='red')
            
            ax2 = ax.twinx()
            ax2.plot(dim_list, snr_list, ms=3, marker='x', lw=1, color='blue', label='snr')
    ...
    def _calc_refl(self, xvar='Q', tol=2):
        ...
        energy_list = self.meta[energy_motor].unique() # List of energies that are in dataset.#
        polarization_list = self.meta[pol_motor].unique() # List of polarizations that are in dataset.
        for pol in polarization_list:
            for en in energy_list:
                column_dict = {'Energy': en, 'Polarization': pol}
                image_subset = [image for image in self.PXRImages if image.energy == en and image.polarization == pol] # Get the images that matter
                ...
                scale_factors = self._calc_scale_factors(stitch_points, normalized_subset)
        ...
        self.normalized_data = pd.concat(self._normalized_data, axis=0).reset_index(drop=True)
        self.stitch_points = pd.concat(self._stitch_points, axis=0).reset_index(drop=True)
        self.scale_factors = pd.concat(self._scale_factors, axis=0).reset_index(drop=True)
    ...
    def _normalize_data(self, images):
        ...
        df_meta = pd.DataFrame({key: [d[key] for d in tmp_meta] for key in tmp_meta[0].keys() if key in variable_motors}) # Get the variable motors in a dataframe.#
        df = pd.concat([df_data, df_meta.round(3)], axis=1)
        # Determine where the images that contain the direct beam
        try:
            i0_cutoff = df.loc[np.abs(df['Sample Z'].diff()) > 0.1].index[0] # The first position where we move from Z out of the way into the beam
        ...
        df['R_err'] = np.sqrt((df['R'] / i0) ** 2 * ((df['R_err'] / df['R']) ** 2 + (i0_err / i0) ** 2))
        df['R'] = df['R'] / i0
    ...
    def _find_stitch_points(self, data_norm):
        ...
        """
        df = data_norm[self.variable_motors] # Only take variable motors.drop(['index', 'Q', 'R', 'R_err'], axis=1)  # Use legacy code
        idx = []
        imotor = []
        skip = False
        skip_count = 0
        ...
        for motor in df.columns:
            for i, val in enumerate(np.diff(df[motor])):
                if skip:
                    ...
                    #idx.append(i)
                    idx.append(i+1) # Edit TJF 1/14 : Moving mark up 1 space to better allow for fitting in later steps
                    imotor.append(motor)
                    skip = True
        ...
        return dfx.drop_duplicates(subset=['mark']) # Remove duplicates if two adjustable motors are moved at the same time
    ...
    def _calc_scale_factors(self, stitch_points, data_norm):
        ...
        if not dfx.empty:
            ...
            idq.append([dfx.motor.iloc[0], refl[dfx.motor.iloc[0]].iloc[0], scale, 0, 0, 0]) # Initialize the wave with the direct beam conditions

            for j, x in enumerate(dfx['mark']):
                qstitch = refl['Q'].iloc[x + 1]  # The Qposition where we indicate the start of a "stitch"
                ...
                for i, val in enumerate(refl['Q'].iloc[x+1:]):
                    if val == qstitch: # I have entered a region where qstitch matches the value
                        qrepeat += 1
                    else: # I am no longer at the same repeated value
                        break

                # Calculate the 'pre' values for the stitch ratio
                for i, val in enumerate(refl['Q'].iloc[:x]): # Start cycling through all values up to "Mark" when repeats happen.
                    if val in refl['Q'].iloc[x + qrepeat:].values and val not in dummy_q_list_pre: # Check if 'val' is ever repeated after the series of repeats (x+qindex)
                        dummy_q_list_pre.append(refl['Q'].iloc[i])
                        dummy_r_list_pre.append(refl['R'].iloc[i]) # Capture the r-value
                        dummy_err_list_pre.append(refl['R_err'].iloc[i]) # Capture the err-value
                # Calculate the post-'Mark' values for stitch ratio
                for i, val in enumerate(refl['Q'].iloc[x+qrepeat:]): # Start cycling through all values
                    # Check if 'val' is ever before 'mark' AND check if we didnt accidently drop it previously from low SNR
                    if val in refl['Q'].iloc[:x].values and val in dummy_q_list_pre and val not in dummy_q_list_post:
                ...
                mydata = RealData(pre_stitch_elements, post_stitch_elements, sx=pre_stitch_error, sy=post_stitch_error)
            ...
            dfx2 = pd.DataFrame(idq, columns=['motor', 'value', 'Scale', 'Scale_Err', 'mark', 'skip'])
            data_scale = dfx2.sort_values(by='mark', ascending=True)
        else:
            data_scale = pd.DataFrame(columns=['motor', 'value', 'Scale', 'Scale_Err', 'mark', 'skip'])
    ...
    def _stitch_refl(self, scale_factors, data_norm, xvar='Q'):
        ...
        for i in range(numpoints):
            ...
            scale_err = scale_factors.Scale_Err.iloc[i]# self.scale_factors.Scale_Err.iloc[i]
            ...
            if i == (numpoints - 1):
                ...
                high_trip = scale_factors.mark.iloc[i + 1]-1 # self.scale_factors.mark.iloc[i + 1]-1
            ...
            for j in range(len(refl_append)):
                ...
                r_err = r_corr * ((refl_append['R_err'].iloc[j] / refl_append['R'].iloc[j]) ** 2 + (
                            scale_err / scale) ** 2) ** 0.5
...
def dezinger_image(image, med_result=None, threshold=10, size=3):
    ...
    zinged_image = image*np.greater(threshold, diff_image).astype(int) + med_result*np.greater(diff_image, threshold) # Repopulate image by removing pixels that exceed the threshold 
