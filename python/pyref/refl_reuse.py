...
class Reuse:
    ...
    def saveForReuse(obj):
        ...
        images = obj.refl.applymap(
            lambda x: x.tolist() if isinstance(x, np.ndarray) else x
        )
        images.to_parquet(
            str(saveDir) + FILE_NAMES["image.parquet"], index=False, compression="gzip"
        )
        obj.refl.to_parquet(
            str(saveDir) + FILE_NAMES["meta.parquet"], index=False, compression="gzip"
        )
        obj.refl.to_json(
            str(saveDir) + FILE_NAMES[".json"], orient="columns", compression="gzip"
        )
    ...
    @staticmethod
    def multiOpen(
        multiDir: Path, energy: str, pol: str
