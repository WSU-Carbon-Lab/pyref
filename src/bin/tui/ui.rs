...
const OPEN_DIR_TITLE_EDIT: &str =
    " [EDIT] type path  Enter apply  Esc back ";
...
fn render_bottom_bar(frame: &mut Frame, app: &App, area: Rect, theme: ThemeMode) {
    ...
    if area.height >= 2 {
        if let Some(ref e) = app.app_error {
            ...
            let hint = e.suggestion.as_deref().unwrap_or("Escape to dismiss.").trim();
            let hint_para = Paragraph::new(Line::from(ratatui::text::Span::styled(
                hint,
                err_style,
            )))
            .alignment(Alignment::Center);
